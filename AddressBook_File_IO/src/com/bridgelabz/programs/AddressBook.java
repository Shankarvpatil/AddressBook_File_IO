...
public class AddressBook {
	public static final ArrayList<ContactData> contactList = new ArrayList<ContactData>();
	public static Map<String, ContactData> nameHashMap = new HashMap<String, ContactData>();
	public static Map<String, ContactData> cityHashMap = new HashMap<String, ContactData>();
	public static Map<String, ContactData> stateHashMap = new HashMap<String, ContactData>();
	static Scanner sc = new Scanner(System.in);
	static AddressBook addressBook = new AddressBook();

	public static boolean addContact(ContactData contact) {
		List<ContactData> checkByName = searchByName(contact.getfName());
		for (ContactData equalName : checkByName) {
	...
	public static List<ContactData> searchByName(String name) {// collection list of element
	...
	public List<ContactData> sortBy(Function<? super ContactData, ? extends String> key) {
		return contactList.stream().sorted(Comparator.comparing(key)).collect(Collectors.toList());
	}

	// method for sort contact by zip
	public List<ContactData> sortByZip(Function<? super ContactData, ? extends Long> key) {
		return contactList.stream().sorted(Comparator.comparing(key)).collect(Collectors.toList());
	}

	// method for edit contact
	public static boolean editContact(ContactData current, ContactData editedContact) {
	...
	public static boolean deleteContact(ContactData contacts) {
	...
	public static ContactData readContact() {
		...
		return new ContactData(firstName, lastName, address, city, state, zip, phoneNum, email);
	}
	...
	public void addressBookOptions(AddressBook addressBook) {
		...
		while (true) {
			...
			case 2:
				...
				List<ContactData> equalName = searchByName(name);// list of equal first name
	...
	public static void sortByOption() {
		...
		switch (choice) {
		case 1:
			addressBook.sortBy(ContactData::getfName).forEach(System.out::println);
			break;
		case 2:
			addressBook.sortBy(ContactData::getlName).forEach(System.out::println);
		
			break;
		case 3:
			addressBook.sortBy(ContactData::getCity).forEach(System.out::println);
			break;
		case 4:
			addressBook.sortBy(ContactData::getState).forEach(System.out::println);
			break;
		case 5:
			addressBook.sortByZip(ContactData::getZip).forEach(System.out::println);
			break;
		case 6:
			return;
		default:
